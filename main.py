...
class PCPost:
    ...
    def login(self):
        try:
            ...
            data = {'username': 'example', 'password': 'example'} #For example: 200119000X (Your Student Number), '123456' (The Last Six Number of Your ID Card)
    ...
    def clock_in(self, cookie):
        ...
        try:
            r = self.session.request('POST', url='http://stu.cugb.edu.cn/syt/zzapply/operation.htm',
                                     headers=self.headers, cookies=cookie_para, data=data)
            #print(r.status_code)
            if r.text == 'success':
